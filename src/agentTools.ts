...
const tools: Record<string, Tool> = {
  ...
  async read_file_section(args, ctx) {
    try {
      const rel = String(args.relPath || '');
      const entry = ctx.scan.files.find((f) => f.rel === rel);
      if (!entry) return { ok: false, error: 'not_found' };
      ...
      const rawEnd = Number.isFinite(Number(args.endLine)) ? Number(args.endLine) : start + 40;
      const end = Math.max(start, Math.min(lines.length, rawEnd));
      const snippet = lines
        .slice(start - 1, end)
        .map((line, idx) => `${start + idx}: ${line}`)
        .join('\n');
      return { ok: true, data: snippet };
    } catch (e: any) {
      return { ok: false, error: String(e) };
    }
  },
  async list_files(args, ctx) {
    ...
    const files = ctx.scan.files
      .filter((f) => (contains ? f.rel.toLowerCase().includes(contains) : true))
      .slice(0, limit)
      .map((f) => ({ rel: f.rel, ext: f.ext, lines: f.lines }));
  ...
  async get_ast_digest(args, ctx) {
    try {
      ...
      return {
        ok: true,
        data: { exports, hasFetch, hasTimers, usesEffect, usesNavigation },
      };
    } catch (e: any) {
      return { ok: false, error: String(e) };
    }
...
export function buildSessionFunctions(ctx: AgentCtx) {
  let toolSteps = 0;
  ...
  return {
    ...
    read_file: wrap(
      ...
      {
        type: 'object',
        properties: { relPath: { type: 'string' }, maxChars: { type: 'number' } },
        required: ['relPath'],
      },
      async (args) => {
    ...
    read_file_section: wrap(
      ...
      async (args) => {
        const r = await tools.read_file_section(args, ctx);
        if (!r.ok) throw new Error(r.error!);
        return r.data;
      },
    ),
    list_files: wrap(
      ...
      async (args) => {
        const r = await tools.list_files(args, ctx);
        if (!r.ok) throw new Error(r.error!);
        return r.data;
      },
    ),
    ...
    get_imports: wrap(
      ...
      async (args) => {
        const r = await tools.get_imports(args, ctx);
        if (!r.ok) throw new Error(r.error!);
        return r.data;
      },
    ),
    ...
    infer_props_from_usage: wrap(
      'infer_props_from_usage',
      {
        type: 'object',
